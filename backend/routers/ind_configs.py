...
_DESCRIPTIONS = {
    ...
    'aVWAP_averages':        "Averages together the currently-active aVWAP lines of whichever anchor type(s) you enable (peaks, valleys, combined peaks+valleys, gaps, order blocks, BoS/CHoCH, QQEMOD, or all of them combined), producing a single smoothed centreline per type. Each type has its own params (periods, max anchors, mode, etc.) plus a shared lookback controlling how many of the most-recent anchors get averaged together. Useful as a dynamic centreline for mean-reversion analysis (see ZScore, Oscillation Volatility) without needing to plot every individual raw anchor line.",
    ...
    'aVWAP_curve_fit':       "Ranks candidate swing-high/low anchors — at one or more swing-detection scales — by how sharply their aVWAP line has curved at its strongest point, rather than by volume concentration (aVWAP — Volume Fit) or touch count (aVWAP — Liquidity Fit). Curve strength is the largest ATR-normalized aVWAP slope an anchor has ever reached, tracked as a running peak — since anchors form at swing extremes, the sharpest move naturally happens early in the anchor's life, so the peak locks in then and stays fixed as the line later flattens. Keeps the top-ranked (sharpest-curving) anchors per side per period, rendered with opacity/width scaled smoothly by rank. First slice of a larger design that will eventually also score how much a curve has since flattened relative to its own peak — deliberately curve detection only for now, to confirm real curves show up clearly before layering timing on top.",
    ...
    'aVWAP_liquidity_fit':   "Ranks candidate swing-high/low anchors — at one or more swing-detection scales — by how many separate times price has come back and touched their line, rather than by recency, variance, or volume concentration (see aVWAP — Volume Fit for that). A liquidity-pool-style read, but using aVWAP anchors as the levels instead of raw swing highs/lows: a contiguous run of bars inside an ATR-based proximity band around the anchor's evolving aVWAP counts as one touch, so the count answers 'how many separate times did price return' rather than 'how many bars was price near it'. Keeps the top-ranked (most-touched) anchors per side per period, rendered with opacity/width scaled smoothly by rank. Also outputs a ticker-level mean touch count, useful for comparing how often a ticker's price retests its own structural levels.",
    ...
    'aVWAP_volume_fit':      "Ranks candidate swing-high/low anchors — at one or more swing-detection scales — by how much of their own trading volume has occurred near their line, rather than by recency or raw variance. For each anchor, computes the fraction of all volume since it formed that traded within an ATR-based band of its evolving aVWAP; a high fraction means the market has consistently, disproportionately transacted near this exact level. Normalized as a fraction (not a raw total) so old anchors don't win just for having existed longer — only anchors the market keeps genuinely returning to score well. Keeps the top-ranked anchors per side per period, rendered with opacity/width scaled smoothly by rank so the best-fit lines stand out at a glance. Also outputs a ticker-level mean volume-fit score, useful for comparing how concentrated a ticker's trading is around its own structural levels versus scattered.",
